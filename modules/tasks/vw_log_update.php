...
echo $AppUI->_('minutes elapsed'); ?>)";
	...
	function timerStart() {
		if (!timerID) { // this means that it needs to be started
			...
			UpdateTimer();
      var now = new Date();
      document.getElementById("StartHour").innerHTML = now.getHours();
      document.getElementById("StartMinute").innerHTML = now.getMinutes();
      document.getElementById("CurrentTimeElapsed").innerHTML = document.editFrm.task_log_hours.value * 60;
      ...
      var now = new Date();
      var startMinutes = document.getElementById("StartHour").innerHTML * 60 + +document.getElementById("StartMinute").innerHTML;
      var currentMinutes = +now.getHours() * 60 + +now.getMinutes();
      var TimeElapsed = (+currentMinutes - +startMinutes);
      var PrevTimeElapsed = document.getElementById("CurrentTimeElapsed").innerHTML;
      var newTimeElapsed = +TimeElapsed + +PrevTimeElapsed;
      var newTimeElapsedFractional = +newTimeElapsed / +60;
      document.getElementById("TimeElapsed").innerHTML = newTimeElapsedFractional;
      console.log(startMinutes, currentMinutes, TimeElapsed, PrevTimeElapsed, newTimeElapsed, newTimeElapsedFractional);
			timerStop();
		}
	}
...
echo $AppUI->_('Reset'); ?>' onclick="javascript:timerReset()" name='timerResetButton' />
          ...
          <span id='CurrentTimeElapsed'></span>
          <span id='StartHour'></span>
          <span id='StartMinute'></span>
